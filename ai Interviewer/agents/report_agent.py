    ...
    def generate_report(self, memory: InterviewMemory) -> InterviewReport:
        ...
        if memory.history:
            ...
            if not getattr(report, "trajectory_analysis", ""):
                report.trajectory_analysis = memory.get_trajectory_trend() if hasattr(memory, "get_trajectory_trend") else "Candidate demonstrated consistent technical performance."
        ...
        if not report.question_reviews or len(report.question_reviews) < len(memory.history):
            logger.info(f"Merging question reviews: history has {len(memory.history)} turns, report LLM output had {len(report.question_reviews or [])}.")
        ...
        if report.question_reviews:
            ...
            for rev in report.question_reviews:
                lines.extend([
                    ...
                    f"Interviewer Expectation: {getattr(rev, 'interviewer_expectation', 'N/A')}",
                    f"Concepts Covered: {', '.join(getattr(rev, 'concepts_covered', [])) if getattr(rev, 'concepts_covered', []) else 'None'}",
                    f"Concepts Partially Covered: {', '.join(getattr(rev, 'concepts_partially_covered', [])) if getattr(rev, 'concepts_partially_covered', []) else 'None'}",
                    f"Missing Concepts: {', '.join(getattr(rev, 'missing_concepts', [])) if getattr(rev, 'missing_concepts', []) else 'None'}",
                    f"What Was Good: {', '.join(rev.what_was_good) if rev.what_was_good else 'N/A'}",
                    f"How To Improve: {rev.how_to_improve}",
                    f"Senior Staff Coaching Diff:\n  {getattr(rev, 'senior_coaching_diff', rev.example_better_answer)}",
                    subborder,
                ])
